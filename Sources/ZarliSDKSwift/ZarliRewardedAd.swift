...
public protocol ZarliRewardedAdDelegate: AnyObject {
...
public struct ZarliReward {
    public let amount: Int
    public let type: String
    
    public static let standard = ZarliReward(amount: 1, type: "reward")
}

public class ZarliRewardedAd {
    ...
    public func load() {
        ZarliLogger.debug("Loading rewarded ad for Unit ID: \(adUnitId)")
    ...
    private func handleBidResponse(_ response: BidResponse) {
        ...
        DispatchQueue.main.async {
            self.isReady = true
            ZarliLogger.debug("Rewarded Ad loaded successfully")
            self.delegate?.adDidLoad(self)
        }
    }
    
    /// Shows the rewarded ad. If viewController is not provided, the SDK will attempt to find the top-most controller.
...
extension ZarliRewardedAd: ZarliWebViewControllerDelegate {
    func webViewControllerDidClose(_ controller: ZarliWebViewController) {
        // Assume reward is earned if the user closes it (for now, simply finishing the ad)
        // In a real implementation, we might wait for a JS callback from the creative.
        // For this demo: reward immediately upon close.
        delegate?.ad(self, didEarnReward: .standard)
